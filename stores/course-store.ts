...
interface CourseStore {
  ...
  // Get course detail (fetches from API if needed, caches result)
  fetchCourseDetail: (externalId: string) => Promise<SavedCourse | null>;

  // Transform saved course + tee name → CourseInfo for round entry
  ...
  persist(
    (set, get) => ({
      ...
      fetchCourseDetail: async (externalId) => {
        ...
        try {
          const res = await fetch(`/api/courses/${externalId}`);
    ...
    }),
    {
      ...
      // Only persist courses (not transient search state)
      partialize: (state) => ({
